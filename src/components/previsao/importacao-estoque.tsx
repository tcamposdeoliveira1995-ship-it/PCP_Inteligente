...
import { parseEstoqueAtual } from "@/lib/parser";
import { ImportacaoStatus } from "@/types/pcp";

interface Props {
  onImportar: (dados: { codigo: string; produto: string; quantidade_atual: number; estoque_minimo: number }[], status: ImportacaoStatus) => void;
}

export function ImportacaoEstoque({ onImportar }: Props) {
  ...
  async function handleArquivo(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setCarregando(true);
    try {
      const resultado = await parseEstoqueAtual(file);
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          <Package size={18} className="text-dourado" />
          Estoque Atual
        </CardTitle>
        <CardDescription>Colunas esperadas: Código, Produto, Quantidade Atual, Estoque Mínimo</CardDescription>
      </CardHeader>
